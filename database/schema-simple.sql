-- WeflutGrid Database Schema (Simple - No Complex Functions)
...
-- Performance indexes
CREATE INDEX IF NOT EXISTS idx_h3_level ON heatmap_cells (h3_level);
CREATE INDEX IF NOT EXISTS idx_country ON heatmap_cells (country_code);
CREATE INDEX IF NOT EXISTS idx_updated ON heatmap_cells (updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_h3_index ON heatmap_cells (h3_index);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_aggregated_h3 ON heatmap_aggregated (h3_index);
CREATE INDEX IF NOT EXISTS idx_aggregated_level ON heatmap_aggregated (h3_level);
CREATE INDEX IF NOT EXISTS idx_aggregated_country ON heatmap_aggregated (country_code);

-- Simple function to refresh aggregated view
...
CREATE INDEX IF NOT EXISTS idx_etl_source ON etl_runs (source_name);
CREATE INDEX IF NOT EXISTS idx_etl_started ON etl_runs (started_at DESC);
